...
function MyPatient() {
  ...
  useEffect(() => {
    const fetchPatients = async () => {
      const res = await getPatientOfDoctor(
        authDoctor?.id,
        limit,
        1,
        debouncedValue.trim(),
      ...
      <div>
        ...
        <div className="mt-5">
          {patients.length > 0 ? (
                      ...
                      >
                        ...
                        <td className="p-3 border-b border-slate-200">
                          <p className="text-sm text-slate-800 flex flex-col gap-1">
                            ...
                            <span className="">
                              SDT: {patient?.patient?.phone}
                            </span>
                            <span>Địa chỉ: {patient?.patient?.address}</span>
                          </p>
                        </td>
                        <td className="p-3 border-b border-slate-200">
                          ...
                          <p className="text-slate-500 flex">
                            <Tippy content="Xem chi tiết">
                              <span
                                ...
                                onClick={() => {
                                  handleShowAppointmentOfPatient(
                                    patient?.id_patient,
                                  );
                                }}
                              >
            ...
            <div className="font-semibold">Không có bệnh nhân nào !</div>
          )}
        </div>
        {patients.length > 5 && (
          <div>
