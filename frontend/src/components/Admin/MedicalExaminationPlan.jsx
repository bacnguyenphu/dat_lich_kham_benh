...
function MedicalExaminationPlan({ type }) {
  ...
  const classNormal =
    "px-5 py-2.5 border rounded-lg text-center cursor-pointer";
  const classActive =
    "px-5 py-2.5 border font-semibold text-white bg-primary-100 rounded-lg text-center cursor-pointer";
      ...
      const fetchDoctors = async () => {
        ...
        if (res.err === 0 && res.data.length > 0) {
          let temp = res.data.map((item) => {
            return {
              value: item?.id,
              label: `${item?.user?.firstName} ${item?.user?.lastName} - Chuyên khoa: ${item?.specialty.map(
                (spe) => spe?.name,
              )}`,
            };
          });
  ...
  useEffect(() => {
    if (selectedItem != null) {
      const tses = {
        [type === "DOCTOR" ? "id_doctor" : "idMedicalPackage"]:
          selectedItem.value,
      ...
      const fetchScheduleFollowDate = async () => {
        ...
        if (res.err === 0 && res.data !== null) {
          let temp = _.cloneDeep(timeFrames).map((item) => {
            return {
              ...item,
              selected: res.data.time_frame.some(
  ...
  return (
    <div className="mt-10 w-5xl mx-auto shadow-item px-4 pb-5">
      {(type === "DOCTOR" || type === "DOCTOR_ONLY") && (
        <h3 className="font-semibold text-2xl text-center text-[#0106B4] border-b border-gray-400 py-5">
          Kế Hoạch Khám Bệnh Của Bác Sĩ
        </h3>
      )}
      {type === "MEDICAL_PACKAGE" && (
        <h3 className="font-semibold text-2xl text-center text-[#0106B4] border-b border-gray-400 py-5">
          Kế Hoạch Khám Bệnh Của Gói Khám
        </h3>
      )}
      <div className="mt-10 flex items-center gap-5">
        <div className="w-9/12">
          {type === "MEDICAL_PACKAGE" && (
            <p className="mb-1">
              Tên gói khám và danh mục <span className="text-red-500">*</span>
            </p>
          )}
          {(type === "DOCTOR" || type === "DOCTOR_ONLY") && (
            <p className="mb-1">
              Tên bác sĩ và chuyên khoa <span className="text-red-500">*</span>
            </p>
          )}
          <Select
        ...
        <div className="w-3/12">
          ...
          <input
            type="date"
            className="border border-gray-300 rounded-md p-1.5 w-full"
            value={selectedDate}
        ...
        {timeFrames.length > 0 &&
          timeFrames.map((item) => {
            return (
              <div
                key={item.id}
                className={item.selected ? classActive : classNormal}
                onClick={() => {
                  handleSelectTimeFrame(item.id);
                }}
              >
                {item?.time_frame}
              </div>
