  ...
  try {
    ...
    const data = await db.Medical_package.findOne({
      include: [
        ...
        {
          model: db.Description_detail,
          as: "description_detail",
...
const getMedicalPackageFollowIdDoctor = async (idDoctor) => {
  try {
    if (!idDoctor) {
      return {
        err: 1,
    ...
    const medical_packages = await db.Medical_package.findAll({
    ...
    return {
      err: 0,
      message: "Get medical package follow doctor success !",
      data: medical_packages,
    };
  } catch (error) {
    console.log("Lỗi ở getMedicalPackageFollowIdDoctor !");
    return {
      err: -999,
      message: `Error server : ${error}`,
    };
