...
const handleChangePasswordDoctor = async (req, res) => {
  try {
    ...
    const message = await changePasswordDoctor(
      idDoctor,
      oldPassword,
      newPassword,
    );
...
const handleLoginReceptionist = async (req, res) => {
  try {
    const data = req.body;
    const message = await loginReceptionist(data);
    const { refreshToken, ...response } = message;
    res.cookie("refreshToken", refreshToken, {
      httpOnly: true,
      sameSite: "none",
    ...
    console.log("Lỗi ở handleLoginReceptionist", error);
    return res.status(200).json({
      err: 999,
      message: "Error Server",
    });
